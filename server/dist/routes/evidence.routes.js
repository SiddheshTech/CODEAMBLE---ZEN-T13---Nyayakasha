...
evidenceRouter.get('/:id/chain', (req, res) => {
    const item = primaryStore.getEvidenceById(req.params.id);
    if (!item) {
        return res.status(404).json({ error: 'Evidence exhibit not found' });
    }
...
evidenceRouter.post('/submit', async (req, res) => {
    try {
        ...
        const newExhibit = {
            ...
            tags,
            evidenceNotes,
            submitterPhotoUrl: req.user?.profilePhotoUrl || 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=400&q=80',
            txHash: anchorResult.txHash,
            blockNumber: anchorResult.blockNumber,
            merkleRoot: anchorResult.merkleRoot,
