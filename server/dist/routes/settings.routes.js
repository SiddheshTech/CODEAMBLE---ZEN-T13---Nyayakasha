...
settingsRoutes.get('/', async (req, res) => {
    try {
        ...
        const clientIp = req.headers['x-forwarded-for'] || req.socket.remoteAddress || '10.208.12.88';
        ...
        const loginHistory = chain
            .filter((b) => b.userId === user.id || b.userId === user.email || b.eventType.includes('AUTH') || b.eventType.includes('LOGIN') || b.eventType.includes('SIGNIN'))
            .slice(-10)
            .reverse()
            .map((b, idx) => ({
            id: `log-${b.index || idx}`,
        ...
        if (loginHistory.length === 0) {
            loginHistory.push({ id: 'log-1', timestamp: 'Today, 09:15 AM', location: 'Fort, Mumbai', device: deviceName, ip: '10.202.4.12', status: 'Success' }, { id: 'log-2', timestamp: 'Yesterday, 04:30 PM', location: 'Chambers Precinct', device: 'Chambers Knox Tablet', ip: '192.168.1.104', status: 'Success' });
        }
        return res.json({
            success: true,
 ...
 */
settingsRoutes.post('/revoke-session', async (req, res) => {
    try {
        const user = await getAuthenticatedUser(req);
        if (!user) {
            return res.status(404).json({ success: false, message: 'User not found' });
