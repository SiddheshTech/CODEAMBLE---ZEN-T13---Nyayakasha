 ...
 */
verificationRouter.post('/document/upload', requireAuth, upload.single('document'), async (req, res) => {
    try {
        if (!req.file) {
 ...
 */
verificationRouter.post('/institution/cross-check', requireAuth, async (req, res) => {
    try {
        const user = await primaryStore.getUserById(req.userId);
        if (!user)
            return res.status(404).json({ error: 'USER_NOT_FOUND' });
 ...
 */
verificationRouter.post('/admin/approve-user', requireAuth, requireRole('court_authority', 'independent_validator'), async (req, res) => {
    try {
        ...
        if (decision === 'APPROVE') {
            ...
            const blockchainRecord = await blockchainService.anchorApprovalEvent(targetUser.id, targetUser.stateHistory[targetUser.stateHistory.length - 1].timestamp, 'mfa_pending');
            return res.json({
 ...
 */
verificationRouter.get('/vetting/queue', requireAuth, requireRole('independent_validator'), async (req, res) => {
    try {
