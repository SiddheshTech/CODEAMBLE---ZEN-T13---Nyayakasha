 ...
 */
export async function getWebAuthnRegisterOptions(user) {
 ...
 */
export function setupTOTPSecret(user) {
 ...
 * Verify & Save TOTP
 */
export async function verifyAndEnrollTOTP(user, secret, token) {
 ...
 */
export function verifyDeviceAttestation(token, platform) {
    if (!token)
        return { valid: false };
