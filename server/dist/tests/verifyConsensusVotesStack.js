async function testConsensusVotesStack() {
    ...
    try {
        ...
        console.log('   - Requests Returned:', pendingData.pendingRequests?.map((r) => `${r.id} (${r.caseRef || r.caseId}) -> Status: ${r.status}`));
        ...
        const invalidVoteData = await invalidVoteRes.json();
