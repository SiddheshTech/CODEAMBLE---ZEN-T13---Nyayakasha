...
import { AuthenticatedRequest } from './roleGuard.js';

// Pre-defined Indian Judicial Jurisdiction Bounding Boxes (Latitude/Longitude ranges)
const JURISDICTION_BOUNDS: Record<string, { minLat: number; maxLat: number; minLng: number; maxLng: number }> = {
 ...
 */
export function verifyJurisdictionGeofence(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  ...
  if (req.userRole === 'field_submitter') {
    ...
    const jurisdictionCode = (req.headers['x-jurisdiction-code'] as string) || req.body.jurisdictionCode || 'DEFAULT_ALL_INDIA';
