...
import { primaryStore, UserRecord, UserRole } from '../db/store.js';
...
authRouter.post('/signup', async (req: Request, res: Response) => {
  try {
    ...
    const validRoles: UserRole[] = ['field_submitter', 'court_authority', 'independent_validator'];
    ...
    if (inviteToken) {
      try {
        const decoded: any = jwt.verify(inviteToken, ENV.JWT_SECRET);
        ...
        if (decoded.jti) await sessionStore.invalidateToken(decoded.jti);
      } catch (err) {
    ...
    notificationService.sendEmail(
      newUser.email,
      'Nyayakasha — Registration Confirmation',
      `<p>Hello <strong>${newUser.fullName}</strong>,</p>
       ...
       <p>Current Status: <strong>Institutional Verification Initiated</strong>.</p>`
    ).catch(err => console.log('Email dispatch error:', err));
 ...
 */
authRouter.get('/session-status', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
 ...
 */
authRouter.post('/enroll-duress-pin', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const user = await primaryStore.getUserById(req.userId!);
    if (!user) return res.status(444).json({ error: 'USER_NOT_FOUND' });
 ...
 */
authRouter.post('/verify-duress-pin', requireAuth, verifyJurisdictionGeofence, async (req: AuthenticatedRequest, res: Response) => {
  try {
 ...
 */
authRouter.post('/invite/generate', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
