 ...
 */
courtAuthorityRouter.get('/dashboard', (req: Request, res: Response) => {
  try {
    ...
    const openCases = cases.filter(c =>
      c.status === 'Active' || c.status === 'Pending Review'
    );

    // Forgery flags requiring action (Under Review)
    const activeForgeryFlags = forgeryReviews.filter(f =>
      f.status === 'Under Review' || f.status === 'Escalated to Bench'
    );

    // Consensus votes awaiting court authority input
    const pendingConsensus = consensusRequests.filter(r =>
      r.status === 'Pending' ||
    ...
    const attentionCount =
      activeForgeryFlags.filter(f => f.status === 'Under Review').length +
 ...
 */
courtAuthorityRouter.get('/attention-items', (req: Request, res: Response) => {
  try {
    primaryStore.loadFromDisk();
    ...
    items.sort((a, b) => (urgencyOrder[a.urgency as keyof typeof urgencyOrder] ?? 3) - (urgencyOrder[b.urgency as keyof typeof urgencyOrder] ?? 3));
 ...
 */
courtAuthorityRouter.get('/recent-activity', (req: Request, res: Response) => {
  try {
    ...
    const activities = relevantEvents.map(e => {
      ...
      if (e.eventType.includes('CONSENSUS') && e.outcome === 'Approved') {
        type = 'vote'; actionLabel = 'Vote Cast'; iconType = 'check';
      } else if (e.eventType.includes('CONSENSUS') && e.outcome === 'Rejected') {
        type = 'vote'; actionLabel = 'Vote Cast'; iconType = 'check';
      } else if (e.eventType.includes('FORGERY')) {
        type = 'ruling'; actionLabel = 'Ruling Issued'; iconType = 'scale';
      } else if (e.eventType.includes('JUDICIAL_ORDER') || e.eventType.includes('ATTESTATION')) {
        type = 'attestation'; actionLabel = 'Attestation'; iconType = 'shield';
      } else if (e.eventType.includes('CUSTODY')) {
        type = 'custody'; actionLabel = 'Custody Transfer'; iconType = 'folder';
      } else if (e.eventType.includes('PRECEDENT')) {
        type = 'precedent'; actionLabel = 'Precedent Resolved'; iconType = 'scale';
      } else if (e.eventType.includes('CASE_DOCKET')) {
        type = 'case'; actionLabel = 'Case Created'; iconType = 'file';
      }
      ...
      if (e.details) {
        if (e.details.requestId) action = `${type === 'vote' ? 'Cast Vote on' : 'Processed'} Consensus Case #${e.details.requestId}`;
        if (e.details.reviewId || e.details.exhibitId) action = `Issued Ruling on Forgery Flag (Case #${e.details.caseId || e.details.reviewId})`;
        if (e.details.flagId) action = `Resolved Precedent Flag #${e.details.flagId} (${e.details.citation || ''})`;
        if (e.details.caseId && e.eventType.includes('CUSTODY')) action = `Approved Custody Transfer for Case #${e.details.caseId}`;
        if (e.details.caseId && e.eventType.includes('CASE_DOCKET')) action = `Created Case Docket #${e.details.caseId}: ${e.details.title || ''}`;
      }
 ...
 */
courtAuthorityRouter.post('/action', async (req: Request, res: Response) => {
  try {
    const {
    ...
    let auditDetails: Record<string, any> = { decision, dbId, caseId, judgeId, judicialOrderText };
    ...
    if (itemType === 'forgery' || decision === 'ADMIT' || decision === 'STRIKE') {
      ...
      req2.nodeVotes = req2.nodeVotes.filter((nv: any) => !nv.nodeRole?.includes('Court Authority'));
      req2.nodeVotes.push({
 ...
 */
courtAuthorityRouter.get('/case-search', (req: Request, res: Response) => {
  try {
    ...
    consensus.filter(r =>
      ...
      (r.exhibitTitle || '').toLowerCase().includes(query)
    ...
    precedents.filter(p =>
      ...
      p.precedentCitation.toLowerCase().includes(query)
