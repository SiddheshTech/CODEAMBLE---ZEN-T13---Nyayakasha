...
import { primaryStore, DirectiveEntry, IdentityUnlockRequest } from '../db/store.js';
...
identityRouter.get('/unlock-requests', (req: Request, res: Response) => {
  const requests = primaryStore.getIdentityUnlocks();
  return res.json({ success: true, requests });
});

// GET a single identity unlock request by ID
identityRouter.get('/unlock-requests/:id', (req: Request, res: Response) => {
  const { id } = req.params;
  const requests = primaryStore.getIdentityUnlocks();
...
identityRouter.post('/submit', (req: Request, res: Response) => {
  const {
    caseId, caseTitle, courtBench, witnessAlias,
    requestingParty, requestingPartyRole, counselBarId, counselAgency,
    statedLegalGrounds, statutoryProvision, urgency,
    witnessRiskIndex, threatAssessmentSummary, protectionCategory
  } = req.body;
  ...
  const newRequest: IdentityUnlockRequest = {
...
identityRouter.post('/directive', (req: Request, res: Response) => {
  ...
  const newDirective: DirectiveEntry = {
