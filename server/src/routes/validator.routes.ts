 ...
 */
validatorRouter.get('/duress-alerts', requireAuth, requireRole('independent_validator', 'court_authority'), async (req: AuthenticatedRequest, res: Response) => {
  try {
 ...
 */
validatorRouter.post('/duress-alerts/:id/acknowledge', requireAuth, requireRole('independent_validator', 'court_authority'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    const user = await primaryStore.getUserById(req.userId!);
    const userName = user ? user.fullName : 'Adv. A. Mehta';
