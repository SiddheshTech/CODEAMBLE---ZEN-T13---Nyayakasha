import { primaryStore, UserRecord, ApprovalState, UserRole } from '../db/store.js';
 ...
 */
export function evaluateDocumentQuality(fileBuffer: Buffer): { blurScore: number; passesQuality: boolean; feedback: string } {
 ...
 */
export async function crossCheckInstitutionalRegistry(user: UserRecord): Promise<{ verified: boolean; registrySource: string; matchDetails: Record<string, any> }> {
 ...
 */
export async function queueValidatorVetting(userId: string, consentGiven: boolean) {
  if (!consentGiven) {
