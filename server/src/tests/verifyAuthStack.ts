...
async function runTests() {
  ...
  const submitterData: any = await submitterRes.json();
  ...
  const courtSignInData: any = await courtSignIn.json();
  ...
  assert(
    Boolean(approveData.blockchainRecord?.txHash) && approveData.blockchainRecord.txHash.startsWith('0x'),
    'Blockchain Anchoring: Account approval event successfully anchored on Polygon PoS chain with valid txHash'
  );
  ...
  const fsSignInData: any = await fsSignIn.json();
  ...
  async function measureLatency(pinToTest: string): Promise<number> {
  ...
  const maxGap = Math.max(
    Math.abs(avgRealLatency - avgDuressLatency),
    Math.abs(avgRealLatency - avgWrongLatency)
  );
  ...
  const valSignInData: any = await valSignIn.json();
  ...
  assert(alerts.length > 0 && alerts.some((a: any) => a.userId === submitterData.userId), 'Duress PIN execution covertly dispatched alert to Independent Validator queue');
